...
/// Default embedded WhatsApp Web gateway URL.
pub const DEFAULT_WHATSAPP_GATEWAY_URL: &str = "http://127.0.0.1:3009";

/// WhatsApp adapter operating mode.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Serialize, Deserialize)]
pub enum WhatsAppMode {
    /// Auto-detect: prefer gateway mode when available, otherwise Cloud API.
    #[default]
    #[serde(rename = "auto")]
    Auto,
    /// Force WhatsApp Cloud API mode.
    #[serde(rename = "cloud_api", alias = "cloud")]
    CloudApi,
    /// Force WhatsApp linked-devices / QR gateway mode.
    #[serde(
        rename = "web_qr",
        alias = "web",
        alias = "gateway",
        alias = "linked_devices"
    )]
    WebQr,
}
...
pub struct WhatsAppConfig {
    /// Adapter mode (`auto`, `web_qr`, `cloud_api`).
    ...
    /// Legacy compatibility: env var name that stores the phone number ID.
    ///
    /// Deprecated in favor of `phone_number_id`, but still supported for
    /// configs generated by older setup flows.
    pub phone_number_id_env: Option<String>,
    /// Port to listen for webhook callbacks (Cloud API mode).
    pub webhook_port: u16,
    /// Optional explicit gateway URL (QR/Web mode), e.g. "http://127.0.0.1:3009".
...
impl WhatsAppConfig {
    /// Resolve effective WhatsApp Web gateway URL from config/env.
    pub fn resolved_gateway_url(&self) -> Option<String> {
        ...
        if !self.gateway_url_env.is_empty() {
            if let Ok(url) = std::env::var(&self.gateway_url_env) {
        ...
        // In explicit Web/QR mode, prefer the embedded default gateway location.
        if self.mode == WhatsAppMode::WebQr {
            return Some(DEFAULT_WHATSAPP_GATEWAY_URL.to_string());
        }

        None
    }

    /// Resolve effective Cloud API phone number ID from config or legacy env indirection.
    pub fn resolved_phone_number_id(&self) -> String {
        if !self.phone_number_id.trim().is_empty() {
            return self.phone_number_id.trim().to_string();
        }
...
impl KernelConfig {
    ...
    pub fn validate(&self) -> Vec<String> {
        ...
        if let Some(ref wa) = self.channels.whatsapp {
            let access_token_set = !std::env::var(&wa.access_token_env)
                .unwrap_or_default()
                .is_empty();
            let verify_token_set = !std::env::var(&wa.verify_token_env)
                .unwrap_or_default()
                .is_empty();
            let phone_number_id_set = !wa.resolved_phone_number_id().is_empty();
            let gateway_set = wa.resolved_gateway_url().is_some();

            match wa.mode {
                WhatsAppMode::CloudApi => {
                    if !access_token_set {
                        warnings.push(format!(
                            "WhatsApp (cloud_api) configured but {} is not set",
                            wa.access_token_env
                        ));
                    }
                    if !verify_token_set {
                    ...
                    if !phone_number_id_set {
                        warnings.push(
                            "WhatsApp (cloud_api) configured but phone_number_id is empty".into(),
                        );
                    }
                }
                WhatsAppMode::WebQr => {
                    if !gateway_set {
                        warnings.push(format!(
                            "WhatsApp (web_qr) configured but gateway URL is missing (set {} or gateway_url)",
                            wa.gateway_url_env
                        ));
                    }
                }
                WhatsAppMode::Auto => {
                    if !gateway_set && !access_token_set {
                        warnings.push(format!(
                            "WhatsApp (auto) configured but neither {} nor {} is set",
                            wa.gateway_url_env, wa.access_token_env
                        ));
                    }
                    if access_token_set && !phone_number_id_set {
                        warnings.push(
                            "WhatsApp (auto/cloud) has access token but phone_number_id is empty"
                                .into(),
                        );
                    }
                }
...
mod tests {
    ...
    fn test_whatsapp_config_defaults() {
        ...
        assert_eq!(wa.mode, WhatsAppMode::Auto);
        assert_eq!(wa.access_token_env, "WHATSAPP_ACCESS_TOKEN");
        assert_eq!(wa.webhook_port, 8443);
        assert_eq!(wa.gateway_url_env, "WHATSAPP_WEB_GATEWAY_URL");
    ...
    fn test_whatsapp_config_serde() {
        ...
        assert_eq!(back.mode, WhatsAppMode::CloudApi);
        assert_eq!(back.phone_number_id, "12345");
    }

    #[test]
    fn test_whatsapp_mode_serde_aliases() {
        let wa: WhatsAppConfig =
            toml::from_str("mode = 'gateway'\nphone_number_id = ''\n").unwrap();
        assert_eq!(wa.mode, WhatsAppMode::WebQr);

        let wa: WhatsAppConfig =
            toml::from_str("mode = 'cloud_api'\nphone_number_id = '123'\n").unwrap();
        assert_eq!(wa.mode, WhatsAppMode::CloudApi);
    }

    #[test]
    fn test_whatsapp_legacy_phone_number_id_env() {
        let var = "OPENFANG_TEST_WA_PHONE_ID";
        // SAFETY: test-scoped env var mutation.
        unsafe {
            std::env::set_var(var, "legacy-phone-id");
        }

        let wa_toml = format!("mode = 'cloud_api'\nphone_number_id_env = '{var}'\n");
        let wa: WhatsAppConfig = toml::from_str(&wa_toml).unwrap();

        assert_eq!(wa.mode, WhatsAppMode::CloudApi);
        assert_eq!(wa.resolved_phone_number_id(), "legacy-phone-id");

        // SAFETY: test cleanup for env var mutation.
        unsafe {
    ...
    #[test]
    fn test_whatsapp_validate_web_qr_without_cloud_token() {
        let mut config = KernelConfig::default();
        config.channels.whatsapp = Some(WhatsAppConfig {
            mode: WhatsAppMode::WebQr,
            ..Default::default()
        });
